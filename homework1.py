...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        print(pd.DataFrame(BigramTableS1, index=BigramTableS1[next(iter(BigramTableS1.keys()))]).transpose())
        print("\nbigram counts table for Sentence 2")
        BigramTableS2 = countingBigramTable(S2,words)
        print(pd.DataFrame(BigramTableS2, index=BigramTableS2[next(iter(BigramTableS2.keys()))]).transpose())
        print("\nbigram probability table for Sentence 1")
        BigramProbS1 = bigramProb(S1, words)
        print(pd.DataFrame(BigramProbS1, index=BigramProbS1[next(iter(BigramProbS1.keys()))]).transpose())
        print("\nbigram probability table for Sentence 2")
        BigramProbS2 = bigramProb(S2, words)
        print(pd.DataFrame(BigramProbS2, index=BigramProbS2[next(iter(BigramProbS2.keys()))]).transpose())
        print("\nbigram counts table for Sentence 1 with smoothing")
        BigramTableSmoothS1 = countingBigramTableSmooth(S1,words)
        print(pd.DataFrame(BigramTableSmoothS1, index=BigramTableSmoothS1[next(iter(BigramTableSmoothS1.keys()))]).transpose())
        print("\nbigram counts table for Sentence 2 with smoothing")
        BigramTableSmoothS2 = countingBigramTableSmooth(S2,words)
        print(pd.DataFrame(BigramTableSmoothS2, index=BigramTableSmoothS2[next(iter(BigramTableSmoothS2.keys()))]).transpose())
        print("\nbigram probability table for Sentence 1 with smoothing")
        BigramProbSmoothS1 = bigramProbSmooth(S1, words)
        print(pd.DataFrame(BigramProbSmoothS1, index=BigramProbSmoothS1[next(iter(BigramProbSmoothS1.keys()))]).transpose())
        print("\nbigram probability table for Sentence 2 with smoothing")
        BigramProbSmoothS2 = bigramProbSmooth(S2, words)
        print(pd.DataFrame(BigramProbSmoothS2, index=BigramProbSmoothS2[next(iter(BigramProbSmoothS2.keys()))]).transpose())
